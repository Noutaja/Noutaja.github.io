...
$(document).ready(function () {
	/* Navbar */ {
		...
		var teamToggleButton = $(
			'<button id="toggleTeamSelStyle" class="btn" data-toggle="button" aria-pressed="false">Team Selector: <span class="glyphicon glyphicon-list"></span><span class="glyphicon glyphicon-th"></span><span> List</span><span> Grid</span></button>'
		)
			.appendTo("#settingsMenu .dropdown-menu")
			.wrap("<li></li>")
		...
		$(
			...
			.click(function (event) {
				...
				if (bigEmotes) $("#messagebuffer").addClass("bigEmotes");
				else $("#messagebuffer").removeClass("bigEmotes");
			});
	...
	/* Tabs */ {
		var tabContainer = $('<div id="MainTabContainer"></div>').appendTo(
			"#videowrap"
		);
		var tabList = $(
			'<ul class="nav nav-tabs" role="tablist"></ul>'
		).appendTo(tabContainer);
		var tabContent = $('<div class="tab-content"></div>').appendTo(
			tabContainer
		);
		...
		var playlistButton = $(
			'<li class="active" role="presentation"><a role="tab" data-toggle="tab" aria-expanded="false" href="#playlistTab">Playlist</a></li>'
		).appendTo(tabList);
		...
		if (getOrDefault(CHANNEL.name + "_I_Am_A_Large_Faggot", false)) {
			$("body").append(
				'<span id="pnl_options" style="position:absolute;display:none;left:0;top:30px;padding-top:10px;width:100%;background:rgba(0,0,0,0.5);z-index:2;"></span>'
			);
			$('<li><a id="btn_playList" class="pointer">Playlist</a></li>')
				.insertAfter("#settingsMenu")
				.click(function () {
		...
		var redoPollwrap = function () {
			...
			$("#pollwrap .well span.label.pull-right")
				.detach()
				.insertBefore("#pollwrap .well h3");
			$("#pollwrap button.close")
				.off("click")
				.click(function () {
					...
					if ($("#pollsTab").hasClass("active") == false) {
						...
						$("#pollsbadge").text(
							(badgeTxt ? parseInt(badgeTxt) : 0) + 1
						);
					}
				});
		};
		...
		Callbacks.newPoll = function (data) {
			base_newPoll(data);
			if (
				$("#pollsTab").hasClass("active") == false &&
				$("#MainTabContainer #pollwrap").length === 0
			) {
				var badgeTxt = $("#pollsbadge").text();
				var pollCnt =
					$("#pollwrap .well.muted").length +
					(badgeTxt ? parseInt(badgeTxt) : 0);
		...
		//Vertical polls
		$(
			'<input class="cs-checkbox" type="checkbox" id="verticalPollsCheckbox" style="float:right;">'
		)
			.prependTo("#pollsTab")
			.click(function () {
				setOpt(CHANNEL.name + "_VERTICAL_POLLS", this.checked);
			});
		$(
			'<label for="verticalPollsCheckbox" style="float:right;padding:1px 8px;">Vertical Polls</label>'
		).prependTo("#pollsTab");
		$("#verticalPollsCheckbox").prop(
			"checked",
			getOrDefault(CHANNEL.name + "_VERTICAL_POLLS", false)
		);

		//Calendar Tab
		$(
			'<div role="tabpanel" class="tab-pane" id="calendarTab"><iframe width="100%" height="600" frameborder="0" scrolling="no"></iframe></div>'
		).appendTo(tabContent);
		$(
		...
		AddCalendar = function (src, color) {
			setOpt(
				CHANNEL.name + "_CALENDARS",
				getOrDefault(CHANNEL.name + "_CALENDARS", []).concat([
					{ src: src, color: color },
				])
			);
		};
		...
		var calendarArgs = calendars
			...
			.join("&");
		$("#calendarTab iframe").attr(
			"src",
			baseCalendarUrl + calendarArgs + "&"
		);
	...
	$("#emotelistbtn")
		...
		.wrap('<div id="emotebtndiv"></div>')
		.text(":^)")
		.attr("title", "Emote List");
	$("#leftcontrols").remove();
	...
	$("#chatline").keydown(function (e) {
		...
		if (!CHATTHROTTLE) {
			...
			if (t.trim() && $("#chatline").val().trim() != previousMessage) {
				var a = {};
				USEROPTS.adminhat && 255 <= CLIENT.rank
					? (t = "/a " + t)
					: USEROPTS.modhat &&
	...
	formatChatMessage = function (data, last) {
		...
		if (data.meta.superadminflair) {
			name.addClass("label").addClass(
				data.meta.superadminflair.labelclass
			);
			$("<span/>")
	...
	trimChatBuffer = function () {
		var doc = $(document),
			currScrollOffset = doc.scrollTop();
		baseTrimChatBuffer();
	...
	handleVideoResize = function () {
		...
		var resize = function () {
			...
			var height =
				responsiveFrame.outerHeight() -
				$("#chatline").outerHeight() -
				2;

			$("#ytapiplayer").attr(
				"height",
				(VHEIGHT = responsiveFrame.outerHeight())
			);
			$("#ytapiplayer").attr(
				"width",
				(VWIDTH = responsiveFrame.outerWidth())
			);
		};
	...
	$("#messagebuffer")
		...
		.click((e) => {
			...
			if (t.className == "channel-emote")
				$("#chatline")
					.val((i, v) => v + " " + e.target.title)
					.focus();
			else if (t.tagName == "IMG") {
				e.preventDefault();
				$('<div id="picoverlay"></div>')
					.click((f) => $("#picoverlay").remove())
					.prependTo("body")
					.append($(p).clone());
				...
				$('<div id="picoverlay"></div>')
					.click((f) => $("#picoverlay").remove())
					.prependTo("body")
					.append($("<video autoplay controls/>").attr("src", t.src));
			}
		});
...
function chatImageToVideo(div) {
	...
	div.find("a>img").each(function (index, img) {
		if (
			...
			if (toReplace.length == 0) toReplace = $(img);
	...
	switch (TimeSetting) {
		case "Countdown":
			...
			if (0 <= diff && diff < 86400)
				return $`0${Math.ceil(diff / 3600)}h0${Math.ceil(
					(diff / 60) % 60
				)}m${Math.ceil(diff % 60)}s`.replaceAll(/0(\d\d)/g, "$1");
		case "UTC":
			return time.toUTCString().substring(17, 22);
		case "Local 24h":
			return time.toTimeString().substring(0, 5);
		case "Local 12h":
			return `0${time.getHours() % 12 || 12}:0${time.getMinutes()} ${
				time.getHours() < 12 ? "AM" : "PM"
			}`.replaceAll(/0(\d\d)/g, "$1");
	}
}
...
function SetCups(height, fontsize, cups) {
	...
	$("#streamtitle").remove();
	$("#videowrap>.embed-responsive").after(
		`<span id="streamtitle" style="font-size:${fontsize}px;">${cupHtmls.join(
			" | "
		)}</span>`
	);
}
function ChallongeTab(challongeLink, tabTitle) {
	...
	if (challongeLink) {
		let challongeTab = $(
			'<div role="tabpanel" class="tab-pane" id="challongeTab"></div>'
		).appendTo("#MainTabContainer>div");
		challongeTab.append(
