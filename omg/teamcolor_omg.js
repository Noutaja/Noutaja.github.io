...
function setTeamList(teamList) {
	...
	if (teamListName != "" && teamListName === CurrentTeamListName) return;
	...
	newSelector.html(
		'<li tabindex="0" data-val=""><img src="' + defaultIconSrc + '"></li>'
	);
	...
	var addOption = function (teamObj) {
		if (!teamObj.ExclusiveTo || teamObj.ExclusiveTo == CLIENT.name) {
			...
			selectedColorInList =
				selectedColorInList || teamObj.id === TEAMCOLOR;
		}
	};
...
function InitTeamLists() {
	var cssLines = ['<style type="text/css" id="iconcss">'];
	...
	var selectteam = $('<div id="selectteam"></div>')
		...
		.append(dropup);
	dropup.append(
		'<img class="dropdown-toggle" data-toggle="dropdown" title="Team Icon">'
	);
	...
	$('<ul class="dropdown-menu"></ul>')
		...
		.on("click", "li", function () {
			...
			setOpt(
				CHANNEL.name + "_TEAMCOLOR_" + CurrentTeamListName,
				TEAMCOLOR
			);
			if (TEAMCOLOR)
				$("#selectteam>span>img").attr("src", AllTeams[TEAMCOLOR].icon);
			else $("#selectteam>span>img").attr("src", defaultIconSrc);
		})
		.on("mouseover", "li", function () {
			if (!scrollingToTeam) this.focus({ preventScroll: true });
		...
		.on("keydown", function (event) {
			...
			switch (event.key) {
				...
				case "Up":
					if (selectteam.hasClass("grid")) {
						...
						if (
							iconsInLastRow != 0 &&
							selected.index() >=
								Math.floor(length / iconsPerRow) * iconsPerRow
						)
							indexAbove += Math.floor(
								(iconsPerRow - iconsInLastRow) / 2
							);
						if (indexAbove >= 0)
							elm = $(selected.parent().children()[indexAbove]);
					} else elm = selected.prev();
					break;
				case "ArrowDown":
				case "Down":
					if (selectteam.hasClass("grid")) {
						...
						if (
							...
							var lastRowAdjustment = Math.floor(
								(iconsPerRow - iconsInLastRow) / 2
							);
							...
							if (indexInLastRow >= lastRowAdjustment)
								elm = $(
									selected.parent().children()[
										indexBelow - lastRowAdjustment
									]
								);
						} else if (indexBelow < selected.siblings().length)
							elm = $(selected.parent().children()[indexBelow]);
					} else elm = selected.next();
					break;
				case "ArrowLeft":
				case "Left":
					if (
						selectteam.hasClass("grid") &&
						selected.index() % iconsPerRow != 0
					)
						elm = selected.prev();
					break;
				case "ArrowRight":
				case "Right":
					if (
						selectteam.hasClass("grid") &&
						(selected.index() + 1) % iconsPerRow != 0
					)
						elm = selected.next();
					break;
				case "Tab":
				...
				default:
					if (event.key.length == 1) {
						var itemSel =
							'li[data-val^="' + event.key.toLowerCase() + '"]';
						elm = $("#selectteam li:focus~" + itemSel).first();
						if (!elm.length)
							elm = $("#selectteam " + itemSel).first();
					}
					break;
			}
			if (elm && elm[0]) {
...
function InitTeam(team) {
	...
	var cssSel =
		(team.ExclusiveTo ? ".chat-msg-" + team.ExclusiveTo : "") +
		".team" +
		team.id +
		" .username";
	team.css =
	...
	if (!AllTeams.hasOwnProperty(team.id)) AllTeams[team.id] = team;
}
	...
	vgl: [
		{
			id: "idolmaster",
			color: "#41DCFF",
			icon: "/w/images/7/7a/%40_icon.png",
		},
		...
		{ id: "fg", color: "#54C2F0", icon: "/w/images/d/da/Fg_icon.png" },
		{
			id: "fgoalter",
			color: "#0b3d8e",
			icon: "/w/images/d/da/Fgoalter_icon.png",
		},
		...
		{ id: "gbfg", color: "#4561BA", icon: "/w/images/4/4f/Gbfg_icon.png" },
		{
			id: "gbpen",
			color: "#E50050",
			icon: "/w/images/5/55/Gbpen_icon.png",
		},
		...
		{ id: "ggg", color: "#CC0000", icon: "/w/images/4/49/Ggg_icon.png" },
		{
			id: "hanny",
			color: "#E11212",
			icon: "/w/images/8/80/Hanny_icon.png",
		},
		{
			id: "hgg2d",
			color: "#5DADEC",
			icon: "/w/images/7/7a/Hgg2d_icon.png",
		},
		{ id: "hsrg", color: "#6a060e", icon: "/w/images/f/f5/Hsrg_icon.png" },
		{
			id: "indie",
			color: "#471f10",
			icon: "/w/images/9/93/Indie_icon.png",
		},
		...
		{ id: "lcg", color: "#7F0000", icon: "/w/images/5/5b/Lcg_icon.png" },
		{
			id: "llsifg",
			color: "#E7147E",
			icon: "/w/images/1/14/Llsifg_icon.png",
		},
		{ id: "mmg", color: "#083D8C", icon: "/w/images/9/9b/Mmg_icon.png" },
		{
			id: "nepgen",
			color: "#4E3483",
			icon: "/w/images/4/43/Nepgen_icon.png",
		},
		...
		{ id: "psg", color: "#00CDBA", icon: "/w/images/9/96/Psg_icon.png" },
		{
			id: "revue",
			color: "#FF0000",
			icon: "/w/images/7/73/Revue_icon.png",
		},
		...
		{ id: "smtg", color: "#FFFF00", icon: "/w/images/8/81/Smtg_icon.png" },
		{
			id: "ss13g",
			color: "#827F7F",
			icon: "/w/images/2/2f/Ss13g_icon.png",
		},
		...
		{ id: "vgt", color: "#BFBFBF", icon: "/w/images/0/0a/Vgt_icon.png" },
		{
			id: "vitagen",
			color: "#000000",
			icon: "/w/images/4/47/Vitagen_icon.png",
		},
	...
	rttc: [
		{
			id: "animeswords",
			color: "#E31B22",
			icon: "/w/images/thumb/b/b4/Animeswords_icon.png/25px-Animeswords_icon.png",
		},
		{
			id: "aniplex",
			color: "#3838A6",
			icon: "/w/images/thumb/0/0f/Aniplex_icon.png/25px-Aniplex_icon.png",
		},
		{
			id: "ccpg",
			color: "#DE2910",
			icon: "/w/images/thumb/f/fd/Ccpg_icon.png/25px-Ccpg_icon.png",
		},
		{
			id: "cute",
			color: "#EBA39D",
			icon: "https://implyingrigged.info/w/images/thumb/6/63/Cute_icon.png/25px-Cute_icon.png",
		},
		{
			id: "cygames",
			color: "#61AFEF",
			icon: "https://implyingrigged.info/w/images/thumb/f/fb/Cygames_icon.png/25px-Cygames_icon.png",
		},
		{
			id: "eroge",
			color: "#FFCC4D",
			icon: "https://implyingrigged.info/w/images/thumb/f/f6/Eroge_icon.png/25px-Eroge_icon.png",
		},
		{
			id: "racism",
			color: "#FFF219",
			icon: "https://implyingrigged.info/w/images/thumb/9/9a/Racism_icon.png/25px-Racism_icon.png",
		},
		{
			id: "skfc",
			color: "#FFFFFF",
			icon: "https://implyingrigged.info/w/images/thumb/b/bc/Skfc_icon.png/25px-Skfc_icon.png",
		},
		{
			id: "tamsoft",
			color: "#023894",
			icon: "https://implyingrigged.info/w/images/thumb/9/9c/Tamsoft_icon.png/25px-Tamsoft_icon.png",
		},
	],
	bag: [
		{
			id: "abydos",
			color: "#62c7de",
			icon: "/w/images/8/85/Abydos_icon.png",
		},
		{
			id: "affection",
			color: "#ffddf4",
			icon: "/w/images/c/c3/Affection_icon.png",
		},
		{ id: "ako", color: "#bbcfe7", icon: "/w/images/a/ad/Ako_icon.png" },
		{
			id: "bacommies",
			color: "#FF0000",
			icon: "/w/images/d/d8/BAcommies_icon.png",
		},
		{
			id: "candc",
			color: "#FFFFFF",
			icon: "/w/images/a/ac/C%26C_icon.png",
		},
		{
			id: "coomindoom",
			color: "#FEBF00",
			icon: "/w/images/e/e3/Coomindoom_icon.png",
		},
		...
		{ id: "gdd", color: "#1DFBD7", icon: "/w/images/0/02/Gdd_icon.png" },
		{
			id: "gehenna",
			color: "#FF2400",
			icon: "/w/images/8/8c/Gehenna_icon.png",
		},
		{
			id: "hyakkiyako",
			color: "#E26EA1",
			icon: "/w/images/0/0d/Hyakkiyako_icon.png",
		},
		{
			id: "millennium",
			color: "#4F85F6",
			icon: "/w/images/b/be/Millennium_icon.png",
		},
		{ id: "npc", color: "#808080", icon: "/w/images/e/e5/Npc_icon.png" },
		{
			id: "pakeji",
			color: "#01d2ff",
			icon: "/w/images/1/13/Pakeji_icon.png",
		},
		{
			id: "sechi",
			color: "#fa9c2a",
			icon: "/w/images/e/e4/Sechi_icon.png",
		},
		{ id: "srt", color: "#5B72A4", icon: "/w/images/f/f4/Srt_icon.png" },
		{
			id: "trinity",
			color: "#8bc2d2",
			icon: "/w/images/0/01/Trinity_icon.png",
		},
	],
